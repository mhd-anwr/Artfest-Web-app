    ...
    <div>
      ...
      <div className="bg-white dark:bg-[#0B2A17] rounded-[18px] shadow-[0_4px_14px_rgba(17,95,50,0.06)] dark:shadow-none border border-[#115F32] dark:border-[#71C247]/25 overflow-hidden mb-8">
        ...
        <div className="p-4 sm:p-5 flex flex-col gap-3">
          {teamData.length === 0 && (
            <p className="text-[#31583F] dark:text-[#8ED06C] text-sm text-center py-8">No teams yet.</p>
          )}
          {teamData.map((team) => (
            <div
              key={team.id}
            ...
            >
              ...
              <div className="flex items-baseline gap-1 shrink-0">
                <span className="text-[#115F32] dark:text-[#71C247] font-extrabold text-base sm:text-lg">{team.totalPoints || 0}</span>
      ...
      <div className="bg-white dark:bg-[#0B2A17] rounded-[18px] shadow-[0_4px_14px_rgba(17,95,50,0.06)] dark:shadow-none border border-[#115F32] dark:border-[#71C247]/25 overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 sm:px-5 py-4 border-b border-[rgba(17,95,50,0.15)] dark:border-[#71C247]/20 bg-white dark:bg-[#0B2A17]">
          <div>
            ...
            <p className="text-[#31583F] dark:text-[#8ED06C] text-xs sm:text-sm mt-0.5">A leaderboard showing top 10 positions in each category.</p>
          </div>
        ...
        <div className="divide-y divide-[rgba(17,95,50,0.15)] dark:divide-[#71C247]/15">
          {(!indData[selectedCat] || indData[selectedCat].length === 0) ? (
            ...
            indData[selectedCat].map((student) => (
              <div key={student.id} className="flex items-center gap-3 px-4 sm:px-5 py-3.5 hover:bg-[#F7FBF5] dark:hover:bg-[#123D22]/40 transition">
                <span className="text-[#31583F] dark:text-[#8ED06C] text-xs font-bold w-5 shrink-0">#{student.rank}</span>
                <div
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-0.5 text-[11px] sm:text-xs text-[#52705D] dark:text-[#8ED06C]">
                    ...
                    <span>•</span>
                    <span className="bg-[#F7FBF5] text-[#173D28] border border-[rgba(17,95,50,0.25)] dark:bg-[#123D22] dark:text-[#D4FFB8] dark:border-[#71C247]/25 px-1.5 py-0.5 rounded text-[10px] font-semibold">{student.category}</span>
                  </div>
                </div>
                <div className="text-right shrink-0">
                  <span className="text-[#115F32] dark:text-[#71C247] font-bold text-sm sm:text-base">{student.totalPoints}</span>
